...
class BotdetectorValidationException implements Exception {
  final List<String> errors;

  BotdetectorValidationException(this.errors);

  @override
  String toString() => 'BotdetectorValidationException: ${errors.join("; ")}';
}
...
/// Bot Detector API client.
///
/// For more information, visit: https://apiverve.com/marketplace/botdetector?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [ua] (required) - The user agent string to analyze (URL encoded) [format: url]
class BotdetectorClient {
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'ua': ValidationRule(type: 'string', required: true, format: 'url'),
  };

  BotdetectorClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/botdetector',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [BotdetectorValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw BotdetectorValidationException(errors);
    }
  }

  /// Execute a request to the Bot Detector API.
  ///
  /// Parameters are validated before sending the request.
  Future<BotdetectorResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw BotdetectorException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return BotdetectorResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw BotdetectorException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw BotdetectorException('Resource not found');
      } else {
        throw BotdetectorException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is BotdetectorException) rethrow;
      throw BotdetectorException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Bot Detector API client.
class BotdetectorException implements Exception {
  final String message;

  BotdetectorException(this.message);

  @override
  String toString() => 'BotdetectorException: $message';
}
